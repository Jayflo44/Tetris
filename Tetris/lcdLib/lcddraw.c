 ...
 *  \param rcol Column to start drawing the character
 *  \param rrow Row to start drawing the character
 *  \param c Character to draw
 *  \param fgColorBGR Foreground color in BGR
 *  \param bgColorBGR Background color in BGR
 */
 void drawChar8x12(u_char rcol, u_char rrow, char c, u_int fgColorBGR, u_int bgColorBGR) {
    u_char oc = c - 0x20; // Offset to character in font array
    lcd_setArea(rcol, rrow, rcol + 7, rrow + 11); // Set the drawing area for 8x12 character
    for (u_char row = 0; row < 12; row++) { // Iterate over each row
        u_char rowData = font_8x12[oc][row]; // Get the row data for the character
        for (u_char col = 0; col < 8; col++) { // Iterate over each bit (column)
            u_int colorBGR = (rowData & (1 << (7 - col))) ? fgColorBGR : bgColorBGR;
            lcd_writeColor(colorBGR); // Write the pixel color
        }
    }
}
/** Draw a string using 8x12 font
 *  \param col Column to start drawing the string
 *  \param row Row to start drawing the string
...
void drawString8x12(u_char col, u_char row, char *string,
                u_int fgColorBGR, u_int bgColorBGR)
{
 u_char cols = col; // Start at the specified column
  while (*string) {
    drawChar8x12(cols, row, *string++, fgColorBGR, bgColorBGR); // Draw each character
    cols += 9; // Advance column position (8 pixels + 1 for spacing)
  }
}
